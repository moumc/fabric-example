...
configtxgen -profile ProductChannel -outputCreateChannelTx ./config/product_channel.tx -channelID $CHANNEL_NAME
if [ "$?" -ne 0 ]; then
...
configtxgen -profile ProductChannel -outputAnchorPeersUpdate ./config/ProductOrg1MSPanchors.tx -channelID $CHANNEL_NAME -asOrg Org1MSP
if [ "$?" -ne 0 ]; then
...
# generate anchor peer transaction
configtxgen -profile ProductChannel -outputAnchorPeersUpdate ./config/ProductOrg3MSPanchors.tx -channelID $CHANNEL_NAME -asOrg Org3MSP
if [ "$?" -ne 0 ]; then
  echo "Failed to generate anchor peer update for Org1MSP..."
  exit 1
fi
...
# generate anchor peer transaction
configtxgen -profile ProductChannel -outputAnchorPeersUpdate ./config/ProductOrg4MSPanchors.tx -channelID $CHANNEL_NAME -asOrg Org4MSP
if [ "$?" -ne 0 ]; then
  echo "Failed to generate anchor peer update for Org1MSP..."
  exit 1
fi
